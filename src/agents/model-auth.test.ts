...
describe("getApiKeyForModel", () => {
  ...
  it("resolves Tabcode API key from env", async () => {
    const previousTabcode = process.env.TABCODE_API_KEY;

    try {
      process.env.TABCODE_API_KEY = "tabcode-test-key\r\n";
      ...
      expect(resolved.apiKey).toBe("tabcode-test-key");
      expect(resolved.source).toContain("TABCODE_API_KEY");
    } finally {
      if (previousTabcode === undefined) {
        delete process.env.TABCODE_API_KEY;
      } else {
        process.env.TABCODE_API_KEY = previousTabcode;
      }
    }
  });

  it("resolves GMN API key from env", async () => {
    const previous = process.env.GMN_CODEX_API_KEY;

    try {
      process.env.GMN_CODEX_API_KEY = "gmn-test-key\r\n";
      ...
      expect(resolved.apiKey).toBe("gmn-test-key");
      expect(resolved.source).toContain("GMN_CODEX_API_KEY");
    } finally {
      if (previous === undefined) {
        delete process.env.GMN_CODEX_API_KEY;
      } else {
        process.env.GMN_CODEX_API_KEY = previous;
      }
    }
  });
