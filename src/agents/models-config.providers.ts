...
import { buildGmnModelDefinition, GMN_BASE_URL } from "./gmn-models.js";
import { buildTabcodeModelDefinition, TABCODE_BASE_URL } from "./tabcode-models.js";
import {
  TOGETHER_BASE_URL,
...
export async function resolveImplicitProviders(params: {
  ...
  const tabcodeKey =
    resolveEnvApiKeyVarName("tabcode") ??
    resolveApiKeyFromProfiles({ provider: "tabcode", store: authStore });
  if (tabcodeKey) {
    providers.tabcode = {
      baseUrl: TABCODE_BASE_URL,
      api: "anthropic-messages",
      apiKey: tabcodeKey,
      models: [buildTabcodeModelDefinition()],
    };
  }

  const gmnKey =
    resolveEnvApiKeyVarName("gmn") ?? resolveApiKeyFromProfiles({ provider: "gmn", store: authStore });
  if (gmnKey) {
    providers.gmn = {
      baseUrl: GMN_BASE_URL,
      api: "openai-responses",
      apiKey: gmnKey,
      models: [buildGmnModelDefinition()],
    };
  }
