...
import {
  ...
  applyTabcodeConfig,
  applyTabcodeProviderConfig,
  applyGmnConfig,
  applyGmnProviderConfig,
  applyXaiConfig,
  ...
  TABCODE_DEFAULT_MODEL_ID,
  TABCODE_DEFAULT_MODEL_REF,
  GMN_DEFAULT_MODEL_ID,
  GMN_DEFAULT_MODEL_REF,
...
describe("applyTabcodeConfig", () => {
  it("adds tabcode provider with correct settings", () => {
    const cfg = applyTabcodeConfig({});
    expect(cfg.models?.providers?.tabcode).toMatchObject({
  ...
  it("sets correct primary model", () => {
    const cfg = applyTabcodeConfig({});
    expect(cfg.agents?.defaults?.model?.primary).toBe(TABCODE_DEFAULT_MODEL_REF);
  });

  it("merges existing tabcode provider models", () => {
    const cfg = applyTabcodeProviderConfig({
      models: {
        providers: {
          tabcode: {
            baseUrl: "https://old.example.com",
    ...
    expect(cfg.models?.providers?.tabcode?.baseUrl).toBe("https://api.tabcode.cc/claude/glm");
    expect(cfg.models?.providers?.tabcode?.api).toBe("anthropic-messages");
    expect(cfg.models?.providers?.tabcode?.apiKey).toBe("old-key");
    const ids = cfg.models?.providers?.tabcode?.models.map((m) => m.id);
    expect(ids).toContain("old-model");
    expect(ids).toContain(TABCODE_DEFAULT_MODEL_ID);
  });
});

describe("applyGmnConfig", () => {
  it("adds GMN provider with correct settings", () => {
    const cfg = applyGmnConfig({});
    expect(cfg.models?.providers?.gmn).toMatchObject({
  ...
  it("sets correct primary model", () => {
    const cfg = applyGmnConfig({});
    expect(cfg.agents?.defaults?.model?.primary).toBe(GMN_DEFAULT_MODEL_REF);
  });

  it("merges existing GMN provider models", () => {
    const cfg = applyGmnProviderConfig({
      models: {
        providers: {
          gmn: {
            baseUrl: "https://old.example.com",
    ...
    expect(cfg.models?.providers?.gmn?.baseUrl).toBe("https://gmn.chuangzuoli.com/v1");
    expect(cfg.models?.providers?.gmn?.api).toBe("openai-responses");
    expect(cfg.models?.providers?.gmn?.apiKey).toBe("old-key");
    const ids = cfg.models?.providers?.gmn?.models.map((m) => m.id);
    expect(ids).toContain("old-model");
    expect(ids).toContain(GMN_DEFAULT_MODEL_ID);
  });
});
