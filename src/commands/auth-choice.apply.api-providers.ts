...
export async function applyAuthChoiceApiProviders(
  ...
  if (authChoice === "tabcode-api-key") {
    if (params.opts?.token && params.opts?.tokenProvider === "tabcode") {
      await setTabcodeApiKey(String(params.opts.token ?? "").trim(), params.agentDir);
    } else {
      const key = await params.prompter.text({
        message: "Enter Tabcode API key",
        validate: (value) => (value?.trim() ? undefined : "Required"),
      });
      await setTabcodeApiKey(String(key ?? "").trim(), params.agentDir);
    }
    nextConfig = applyAuthProfileConfig(nextConfig, {
      profileId: "tabcode:default",
      provider: "tabcode",
      mode: "api_key",
    });
    {
      const applied = await applyDefaultModelChoice({
        config: nextConfig,
        setDefaultModel: params.setDefaultModel,
        defaultModel: TABCODE_DEFAULT_MODEL_REF,
        applyDefaultConfig: applyTabcodeConfig,
        applyProviderConfig: applyTabcodeProviderConfig,
        noteDefault: TABCODE_DEFAULT_MODEL_REF,
        noteAgentModel,
        prompter: params.prompter,
      });
      nextConfig = applied.config;
  ...
  if (authChoice === "gmn-api-key") {
    if (params.opts?.token && params.opts?.tokenProvider === "gmn") {
      await setGmnApiKey(String(params.opts.token ?? "").trim(), params.agentDir);
    } else {
      const key = await params.prompter.text({
        message: "Enter GMN API key",
        validate: (value) => (value?.trim() ? undefined : "Required"),
      });
      await setGmnApiKey(String(key ?? "").trim(), params.agentDir);
    }
    nextConfig = applyAuthProfileConfig(nextConfig, {
      profileId: "gmn:default",
      provider: "gmn",
      mode: "api_key",
    });
    {
      const applied = await applyDefaultModelChoice({
        config: nextConfig,
        setDefaultModel: params.setDefaultModel,
        defaultModel: GMN_DEFAULT_MODEL_REF,
        applyDefaultConfig: applyGmnConfig,
        applyProviderConfig: applyGmnProviderConfig,
        noteDefault: GMN_DEFAULT_MODEL_REF,
        noteAgentModel,
        prompter: params.prompter,
      });
      nextConfig = applied.config;
