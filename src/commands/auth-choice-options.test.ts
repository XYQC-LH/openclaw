...
describe("buildAuthChoiceOptions", () => {
  ...
  it("includes Tabcode auth choice", () => {
    const store: AuthProfileStore = { version: 1, profiles: {} };
    const options = buildAuthChoiceOptions({
      store,
      includeSkip: false,
    ...
    expect(options.some((opt) => opt.value === "tabcode-api-key")).toBe(true);
  });

  it("includes GMN auth choice", () => {
    const store: AuthProfileStore = { version: 1, profiles: {} };
    const options = buildAuthChoiceOptions({
      store,
      includeSkip: false,
    ...
    expect(options.some((opt) => opt.value === "gmn-api-key")).toBe(true);
  });
