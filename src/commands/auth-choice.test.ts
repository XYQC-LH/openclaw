...
describe("applyAuthChoice", () => {
  ...
  it("prompts and writes Tabcode API key when selecting tabcode-api-key", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-auth-"));
    process.env.OPENCLAW_STATE_DIR = tempStateDir;
    process.env.OPENCLAW_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPENCLAW_AGENT_DIR;

    const text = vi.fn().mockResolvedValue("sk-tabcode-test");
    const select: WizardPrompter["select"] = vi.fn(
      async (params) => params.options[0]?.value as never,
    );
    const multiselect: WizardPrompter["multiselect"] = vi.fn(async () => []);
    ...
    expect(text).toHaveBeenCalledWith(
      expect.objectContaining({ message: "Enter Tabcode API key" }),
    );
    expect(result.config.auth?.profiles?.["tabcode:default"]).toMatchObject({
      provider: "tabcode",
      mode: "api_key",
    });
    ...
    expect(parsed.profiles?.["tabcode:default"]?.key).toBe("sk-tabcode-test");
  });

  it("prompts and writes GMN API key when selecting gmn-api-key", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-auth-"));
    process.env.OPENCLAW_STATE_DIR = tempStateDir;
    process.env.OPENCLAW_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPENCLAW_AGENT_DIR;

    const text = vi.fn().mockResolvedValue("sk-gmn-test");
    const select: WizardPrompter["select"] = vi.fn(
      async (params) => params.options[0]?.value as never,
    );
    const multiselect: WizardPrompter["multiselect"] = vi.fn(async () => []);
    ...
    expect(text).toHaveBeenCalledWith(expect.objectContaining({ message: "Enter GMN API key" }));
    expect(result.config.auth?.profiles?.["gmn:default"]).toMatchObject({
      provider: "gmn",
      mode: "api_key",
    });
    ...
    expect(parsed.profiles?.["gmn:default"]?.key).toBe("sk-gmn-test");
  });
