...
import {
  buildGmnModelDefinition,
  GMN_BASE_URL,
  GMN_DEFAULT_MODEL_ID,
  GMN_DEFAULT_MODEL_REF,
} from "../agents/gmn-models.js";
import {
  buildTabcodeModelDefinition,
  TABCODE_BASE_URL,
  TABCODE_DEFAULT_MODEL_ID,
  TABCODE_DEFAULT_MODEL_REF,
} from "../agents/tabcode-models.js";
import {
...
export function applyTabcodeProviderConfig(cfg: OpenClawConfig): OpenClawConfig {
  const models = { ...cfg.agents?.defaults?.models };
  models[TABCODE_DEFAULT_MODEL_REF] = {
    ...models[TABCODE_DEFAULT_MODEL_REF],
    alias: models[TABCODE_DEFAULT_MODEL_REF]?.alias ?? "GLM 4.7 (Tabcode)",
  };

  const providers = { ...cfg.models?.providers };
  const existingProvider = providers.tabcode;
  const existingModels = Array.isArray(existingProvider?.models) ? existingProvider.models : [];
  const defaultModel = buildTabcodeModelDefinition();
  const hasDefaultModel = existingModels.some((model) => model.id === TABCODE_DEFAULT_MODEL_ID);
  const mergedModels =
...
export function applyTabcodeConfig(cfg: OpenClawConfig): OpenClawConfig {
  const next = applyTabcodeProviderConfig(cfg);
  const existingModel = next.agents?.defaults?.model;
  return {
    ...next,
    agents: {
      ...
      defaults: {
        ...
        model: {
          ...(existingModel && "fallbacks" in (existingModel as Record<string, unknown>)
            ...
            : undefined),
          primary: TABCODE_DEFAULT_MODEL_REF,
        },
      },
    },
  };
...
export function applyGmnProviderConfig(cfg: OpenClawConfig): OpenClawConfig {
  const models = { ...cfg.agents?.defaults?.models };
  models[GMN_DEFAULT_MODEL_REF] = {
    ...models[GMN_DEFAULT_MODEL_REF],
    alias: models[GMN_DEFAULT_MODEL_REF]?.alias ?? "Codex (GMN)",
  };

  const providers = { ...cfg.models?.providers };
  const existingProvider = providers.gmn;
  const existingModels = Array.isArray(existingProvider?.models) ? existingProvider.models : [];
  const defaultModel = buildGmnModelDefinition();
  const hasDefaultModel = existingModels.some((model) => model.id === GMN_DEFAULT_MODEL_ID);
  const mergedModels =
    existingModels.length > 0
      ? hasDefaultModel
        ? existingModels
...
export function applyGmnConfig(cfg: OpenClawConfig): OpenClawConfig {
  const next = applyGmnProviderConfig(cfg);
  const existingModel = next.agents?.defaults?.model;
  return {
    ...next,
    agents: {
